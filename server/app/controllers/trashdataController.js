...
exports.create=(req,res) => {
  const trashpost = new trashdata({
    ...
    tempat_sampah_isfull:req.body.tempat_sampah_isfull ? req.body.tempat_sampah_isfull : false
  })

  trashpost.save(trashpost)
  .then((result)=>{
    res.send(result)
  }).catch((err)=>{
    res.status(409).send({
...
exports.update= (req,res) =>{
  ...
  trashdata.findByIdAndUpdate(id,req.body)
  .then((result)=>{
    if(!result){
      res.status(404).send({
        message : "Not Found"
...
exports.delete= (req,res) =>{
  ...
  trashdata.findByIdAndRemove(id)
  .then((result)=>{
    if(!result){
      res.status(404).send({
        message : "Not Found"
